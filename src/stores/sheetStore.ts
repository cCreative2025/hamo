...
export const useSheetStore = create<SheetStore>((set, get) => ({
  ...
  loadSheets: async (teamId?: string, userId?: string) => {
    ...
    try {
      let query = supabase
        .from('sheets')
        .select('*, sheet_versions(id, file_path, file_type, file_size, page_count, version_number, uploaded_by, created_at), song_forms(id, name, key, tempo, chord_progression, sections, flow, drawing_data, memo, created_by, created_at, updated_at)')
